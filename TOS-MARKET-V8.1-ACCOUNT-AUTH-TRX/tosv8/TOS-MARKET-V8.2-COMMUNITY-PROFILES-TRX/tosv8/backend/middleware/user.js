...
async function optionalUserAuth(req,res,next){try{const token=req.cookies.tos_user;if(token&&process.env.JWT_SECRET){const p=jwt.verify(token,process.env.JWT_SECRET);const {rows}=await db.query('SELECT id,nickname,email,display_name,bio,avatar_mime,created_at FROM users WHERE id=$1',[p.id]);if(rows[0])req.user=rows[0];}}catch{}next();}
