...
## V8.3.1 Avatar Cache Fix
Avatar ve profil kapakları artık `no-store` ile servis edilir ve medya URL'lerine sürüm parametresi eklenir. Profil fotoğrafı değişince eski tarayıcı cache'inin kalması engellenir.
# TOS MARKET v8 — PRO UI / TRX Marketplace
...
- TOS AI: `OPENAI_API_KEY` verilirse OpenAI Responses API üzerinden gerçek AI; anahtar yoksa yerleşik akıllı destek fallback'i
...
- TR / EN / DE dil seçeneği; bayraklı dil seçici
- Her kullanıcıya özel Profil ID, avatar yükleme/kaldırma, public profil ve sosyal medya bağlantıları (Instagram, YouTube, Facebook, Discord, TikTok, Twitter/X, Snapchat)
- Topluluk kullanıcı dizini ve profil bazlı leaderboard görünümü
...
Build marker: `8.3.0-community-profiles`. `/health` ve `/api/version` üzerinden doğrulanabilir.

Not: Sosyal bağlantı `https://t.me/tosmarkt` olarak ayarlı. Bu kullanıcı adı sana aitse doğrudan kullanılır; farklıysa ilgili Telegram linklerini değiştir.


## 8.3 profile media
- Public user directory removed from the customer UI.
- User management is available only in the private Control Room.
- Profiles support avatar plus image/GIF/MP4/WebM cover media.
- Social profile cards use branded icon assets, including Telegram.
- Language selector is Turkish / English / German and observes dynamically rendered site content.


## 8.5 admin improvements
- Private Control Room remains at `ADMIN_PATH` and includes a persistent Site Preview iframe, so admins can browse the public site without leaving the panel.
- User moderation supports active, suspended and banned states, profile visibility, moderation notes, avatar/cover removal, and online/last-seen indicators.
- User activity uses `last_seen_at`; moderation fields are stored in PostgreSQL.
