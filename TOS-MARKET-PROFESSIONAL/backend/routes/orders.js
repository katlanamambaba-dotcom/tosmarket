...
router.post('/',async(req,res)=>{
  ...
  try{
    ...
    const {rows:o}=await client.query('INSERT INTO orders(customer_name,customer_email,total,note) VALUES($1,$2,$3,$4) RETURNING id',[name.trim(),email.trim().toLowerCase(),total,note]);
    ...
    res.status(201).json({orderId:o[0].id,total,adminEmail:process.env.ADMIN_EMAIL||'info@tos.quest',mailed});
