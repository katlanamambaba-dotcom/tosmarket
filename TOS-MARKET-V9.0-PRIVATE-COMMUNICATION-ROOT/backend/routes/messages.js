...
router.get('/conversations',userAuth,async(req,res)=>{
  try{
    const {rows}=await db.query(`
      ...
      WHERE u.account_status='active' AND COALESCE(up.profile_public,TRUE)=TRUE
...
router.get('/user/:userId',userAuth,async(req,res)=>{
  try{
    ...
    const u=await db.query(`SELECT u.id,u.nickname,u.display_name,u.avatar_mime,u.updated_at,(u.avatar_data IS NOT NULL) has_avatar,COALESCE(up.profile_public,TRUE) profile_public FROM users u LEFT JOIN user_preferences up ON up.user_id=u.id WHERE u.id=$1 AND u.account_status='active'`,[otherId]);
    if(!u.rows[0]||!u.rows[0].profile_public)return res.status(404).json({error:'Bu kullanıcı profili gizli.'});
