...
router.post('/',optionalUserAuth,async(req,res)=>{
  const {items=[],paymentMethod='crypto_trx',nickname,couponCode}=req.body||{};
  ...
  try{await client.query('BEGIN');let total=0,verified=[];for(const item of items){const {rows:p}=await client.query('SELECT id,name,description,price,stock,status FROM products WHERE id=$1 FOR UPDATE',[Number(item.productId)]);if(!p[0]||p[0].status!=='active')throw new Error('Bir ürün artık satışta değil.');const qty=Math.max(1,Math.floor(Number(item.quantity)||1));if(qty>p[0].stock)throw new Error(`Yeterli stok yok: ${p[0].name}`);total+=Number(p[0].price)*qty;verified.push({id:p[0].id,name:p[0].name,price:Number(p[0].price),qty});}
    ...
    if(couponCode){const cr=await client.query("SELECT * FROM coupons WHERE code=UPPER($1) AND active=TRUE AND (starts_at IS NULL OR starts_at<=NOW()) AND (ends_at IS NULL OR ends_at>=NOW()) FOR UPDATE",[String(couponCode).trim()]);if(!cr.rows[0])throw new Error('Kupon kodu geçersiz veya süresi dolmuş.');coupon=cr.rows[0];if(Number(total)<Number(coupon.min_total))throw new Error('Bu kupon için sepet toplamı yeterli değil.');if(coupon.usage_limit!==null&&Number(coupon.used_count)>=Number(coupon.usage_limit))throw new Error('Kupon kullanım limiti dolmuş.');discount=coupon.discount_type==='percent'?total*(Number(coupon.discount_value)/100):Number(coupon.discount_value);discount=Math.min(total,Math.max(0,discount));}
    ...
    const {rows:o}=await client.query(`INSERT INTO orders(customer_name,customer_email,total,payment_method,note,payment_token,user_id,customer_nickname,coupon_code,discount_amount) VALUES($1,$2,$3,'crypto_trx','',$4,$5,$6,$7,$8) RETURNING id,payment_token,total,created_at`,[safeNick,userId?req.user.email:null,total,paymentToken,userId,safeNick,coupon?.code||null,discount]);
    if(coupon)await client.query('UPDATE coupons SET used_count=used_count+1 WHERE id=$1',[coupon.id]);
    if(coupon)await client.query('INSERT INTO coupon_redemptions(coupon_id,user_id,order_id) VALUES($1,$2,$3)',[coupon.id,userId,o[0].id]);
    if(userId)await client.query("INSERT INTO notifications(user_id,type,title,body,link) VALUES($1,'order','Sipariş oluşturuldu',$2,$3)",[userId,`#${o[0].id} numaralı siparişin oluşturuldu.`, `/order.html?id=${o[0].id}&token=${o[0].payment_token}`]);
