...
CREATE TABLE IF NOT EXISTS products (
 ...
 image_url VARCHAR(500), image_data BYTEA, image_mime VARCHAR(100), delivery_text TEXT, instant_delivery BOOLEAN NOT NULL DEFAULT FALSE, delivery_time_text VARCHAR(80), external_platform VARCHAR(30), external_url VARCHAR(500),
...
-- Product catalog deletion is safe: historical order item snapshots keep product_name/unit_price.
...
ALTER TABLE products ADD COLUMN IF NOT EXISTS instant_delivery BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE products ADD COLUMN IF NOT EXISTS delivery_time_text VARCHAR(80);
ALTER TABLE products ADD COLUMN IF NOT EXISTS external_platform VARCHAR(30);
ALTER TABLE products ADD COLUMN IF NOT EXISTS external_url VARCHAR(500);
...
-- Platform category normalization for the starter catalog.
UPDATE products SET category='Instagram' WHERE name ILIKE 'Instagram %' AND category='Sosyal';
UPDATE products SET category='TikTok' WHERE name ILIKE 'TikTok %' AND category='Sosyal';
UPDATE products SET category='PSN' WHERE name ILIKE '%PSN%' AND category='Oyun';
UPDATE products SET category='Steam' WHERE name ILIKE '%Steam%' AND category='Oyun';
UPDATE products SET category='Twitch' WHERE name ILIKE '%Twitch%' AND category='Sosyal';
UPDATE products SET category='Telegram' WHERE name ILIKE '%Telegram%' AND category='Sosyal';
UPDATE products SET category='Twitter' WHERE (name ILIKE '%Twitter%' OR name ILIKE '% X %') AND category='Sosyal';
UPDATE products SET category='Facebook' WHERE name ILIKE '%Facebook%' AND category='Sosyal';
UPDATE products SET category='Snapchat' WHERE name ILIKE '%Snapchat%' AND category='Sosyal';
