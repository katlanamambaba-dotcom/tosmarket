...
router.post('/register',async(req,res)=>{
 ...
 try{
  ...
  const {rows}=await client.query(`INSERT INTO users(nickname,email,password_hash,display_name,email_verified,email_verification_code_hash,email_verification_expires_at) VALUES($1,$2,$3,$1,TRUE,NULL,NULL) RETURNING id,nickname,email,display_name,created_at`,[nickname.toLowerCase(),email,hash]);
...
router.post('/login',async(req,res)=>{try{
 ...
 await db.query('UPDATE users SET last_seen_at=NOW(),email_verified=TRUE WHERE id=$1',[rows[0].id]);
