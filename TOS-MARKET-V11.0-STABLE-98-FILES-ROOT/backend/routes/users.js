...
router.get('/me',userAuth,async(req,res)=>{
 try{
  let profilePublic=true, socials=[];
  try{const pref=await db.query('SELECT profile_public FROM user_preferences WHERE user_id=$1',[req.user.id]);profilePublic=pref.rows[0]?.profile_public!==false;}catch{}
  try{const socialRows=await db.query('SELECT platform,url FROM user_socials WHERE user_id=$1 ORDER BY platform',[req.user.id]);socials=socialRows.rows;}catch{}
  const u={...req.user,profile_public:profilePublic,socials,avatar_url:req.user.avatar_mime?mediaUrl('avatar',req.user.id,req.user.updated_at,req.user.avatar_mime):null,cover_url:req.user.cover_mime?mediaUrl('cover',req.user.id,req.user.updated_at,req.user.cover_mime):null};
  res.json({user:u});
 }catch(e){console.error('me:',e.message);res.status(500).json({error:'Hesap bilgileri yüklenemedi.'});}
});
