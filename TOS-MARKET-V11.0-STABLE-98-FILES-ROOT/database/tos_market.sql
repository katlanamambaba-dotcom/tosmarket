...
ALTER TABLE users ADD COLUMN IF NOT EXISTS cover_data BYTEA;
ALTER TABLE users ADD COLUMN IF NOT EXISTS cover_mime VARCHAR(100);
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS moderation_note VARCHAR(500) NOT NULL DEFAULT '';
...
ALTER TABLE products ADD COLUMN IF NOT EXISTS supplier_source TEXT;
...
ALTER TABLE messages ADD COLUMN IF NOT EXISTS guest_token VARCHAR(64);
ALTER TABLE messages ADD COLUMN IF NOT EXISTS user_id BIGINT;
ALTER TABLE messages ADD COLUMN IF NOT EXISTS order_id BIGINT;
ALTER TABLE messages ADD COLUMN IF NOT EXISTS sender_user_id BIGINT;
ALTER TABLE messages ADD COLUMN IF NOT EXISTS recipient_user_id BIGINT;
CREATE INDEX IF NOT EXISTS messages_sender_recipient_idx ON messages(sender_user_id,recipient_user_id,created_at DESC);
CREATE INDEX IF NOT EXISTS messages_recipient_unread_idx ON messages(recipient_user_id,is_read,created_at DESC);
ALTER TABLE messages ALTER COLUMN customer_email DROP NOT NULL;
CREATE INDEX IF NOT EXISTS messages_created_at_idx ON messages(created_at DESC); CREATE INDEX IF NOT EXISTS messages_thread_id_idx ON messages(thread_id); CREATE INDEX IF NOT EXISTS messages_guest_token_idx ON messages(guest_token); CREATE INDEX IF NOT EXISTS messages_user_order_idx ON messages(user_id,order_id,created_at DESC);

CREATE TABLE IF NOT EXISTS auth_logs (
 id BIGSERIAL PRIMARY KEY, user_id BIGINT, action VARCHAR(40) NOT NULL, details VARCHAR(500), created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS auth_logs_created_at_idx ON auth_logs(created_at DESC);
CREATE INDEX IF NOT EXISTS auth_logs_user_idx ON auth_logs(user_id,created_at DESC);
...
-- TOS MARKET V11 operational layer
...
CREATE INDEX IF NOT EXISTS favorites_product_idx ON favorites(product_id, created_at DESC);
...
CREATE TABLE IF NOT EXISTS banners (
 ...
 cta VARCHAR(80) NOT NULL DEFAULT 'Keşfet',
...
CREATE INDEX IF NOT EXISTS inventory_events_product_idx ON inventory_events(product_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS product_views_product_idx ON product_views(product_id, created_at DESC);

INSERT INTO coupons(code,discount_type,discount_value,min_total,active) VALUES
('TOS10','percent',10,100,TRUE),('WELCOME50','fixed',50,500,TRUE)
