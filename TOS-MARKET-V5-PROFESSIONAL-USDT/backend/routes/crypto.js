...
function makeAmount(tl){if(!RATE||RATE<=0)throw new Error('USDT kuru henüz yapılandırılmadı.');const base=toMicro(Number(tl)/RATE);const extra=BigInt(1+Math.floor(Math.random()*999));return base+extra;}
...
async function checkPayment(payment){
  ...
  const data=await tron(q); const rows=Array.isArray(data.data)?data.data:[];
  for(const tx of rows){
    ...
    const amount=BigInt(String(tx.value||'0')); if(amount!==toMicro(payment.expected_amount))continue;
    const hash=tx.transaction_id;if(!hash)continue;
    const used=await db.query('SELECT id FROM crypto_payments WHERE tx_hash=$1',[hash]);if(used.rows.length)continue;
    const solid=await tronPost('/walletsolidity/gettransactionbyid',{value:hash}).catch(()=>null);if(!solid||!solid.txID)continue;
    if(Array.isArray(solid.ret)&&solid.ret[0]?.contractRet&&solid.ret[0].contractRet!=='SUCCESS')continue;
    return {hash,received_amount:formatUsdt(amount)};
  }
  return null;
}
router.post('/create',async(req,res)=>{try{
  if(!PAYMENT_ADDRESS)return res.status(503).json({error:'USDT ödeme adresi yapılandırılmamış.'});
  const {orderId,token}=req.body||{}; if(!orderId||!token)return res.status(400).json({error:'Sipariş anahtarı eksik.'});
  const {rows}=await db.query('SELECT * FROM orders WHERE id=$1 AND payment_token=$2',[orderId,token]);const order=rows[0];
  if(!order)return res.status(404).json({error:'Sipariş bulunamadı.'});
  if(order.payment_method!=='crypto_trc20')return res.status(400).json({error:'Bu sipariş kripto ödemesi değil.'});
  const existing=await db.query('SELECT * FROM crypto_payments WHERE order_id=$1',[order.id]);if(existing.rows[0])return res.json({payment:existing.rows[0],address:PAYMENT_ADDRESS,network:'TRC20',token:'USDT'});
  const amount=formatUsdt(makeAmount(order.total)); const expires=new Date(Date.now()+EXPIRY_MINUTES*60_000);
...
router.get('/status/:orderId',async(req,res)=>{try{
  ...
  if(payment.status==='waiting'&&new Date(payment.expires_at)>new Date()){const found=await checkPayment(payment);if(found){
